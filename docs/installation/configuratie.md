...
| `ELASTIC_EXCLUDED_FIELDS_KENNISBANK`    | Velden waar door een Kennisbank gebruiker niet in gezocht mag worden. <details><summary>Meer informatie </summary>Als deze variabele gespecificeerd is dan kan een Kennisbank gebruiker niet in deze velden zoeken. De specifieke velden zullen ook niet getoond worden aan de gebruiker. Veldnamen moeten gespecificeerd worden zoals ze geconfigureerd zijn in Elasticsearch. Meerdere velden kunnen opgegeven worden, gescheiden door een comma. Bijvoorbeeld: `VAC.toelichting,Kennisbank.vertalingen.deskMemo` </details> |

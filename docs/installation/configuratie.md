...
| `ELASTIC_EXCLUDED_FIELDS_KENNISBANK`    | Velden die een Kennisbank-gebruiker niet mag doorzoeken en inzien. <details><summary>Meer informatie </summary>Als deze variabele gespecificeerd is dan kan een Kennisbank-gebruiker niet in deze velden zoeken. De specifieke velden zijn ook niet zichtbaar voor de gebruiker. Men moet de veldnamen specificeren zoals ze geconfigureerd zijn in Elasticsearch. Men kan meerdere velden specificeren, gescheiden door een komma. Bijvoorbeeld: `VAC.toelichting,Kennisbank.vertalingen.deskMemo` </details> |
